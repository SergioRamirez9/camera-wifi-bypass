...
def main():
    try:
        ...
        print("\n=== Searching scripts for poweroff or fbsplash ===")
        print(run_command(s, "grep -rn 'poweroff' /customer/ /etc/ 2>/dev/null"))
        print(run_command(s, "grep -rn 'fbsplash' /customer/ /etc/ 2>/dev/null"))
        print(run_command(s, "grep -rn 'logo' /customer/ /etc/ 2>/dev/null"))
