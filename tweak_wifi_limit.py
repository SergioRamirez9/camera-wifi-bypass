...
def main():
    try:
        ...
        # 1. Backup the original hostapd.conf
        print("[*] Creating backup of hostapd.conf...")
        run_command(s, "cp -n /customer/wifi/hostapd.conf /customer/wifi/hostapd.conf.bak")
        
        # 2. Use sed to change max_num_sta=1 to max_num_sta=10
        print("[*] Tweaking max_num_sta limit in hostapd.conf...")
        run_command(s, "sed -i 's/max_num_sta=1/max_num_sta=10/g' /customer/wifi/hostapd.conf")
        
        # Verify the change
        print("[*] Verifying hostapd.conf modifications...")
        print(run_command(s, "cat /customer/wifi/hostapd.conf"))
        
        s.close()
        print("[+] Wi-Fi connection limit increased to 10 devices! Please reboot the camera to apply changes.")
